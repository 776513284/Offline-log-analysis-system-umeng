package com.huangsuhai.app.client;

import com.alibaba.fastjson.JSONObject;
import com.huangsuhai.app.common.*;
import com.huangsuhai.app.util.PropertiesUtil;
		...
		try {
			...
			for (int i = 1; i <= 200000000; i++) {
				...
				try {
					...
					AppEventLog[] logArr2 = logEntity.getAppEventLogs();
					for (AppEventLog log : logArr2) {
						PropertiesUtil.copyProperties(logEntity, log);
						log.setCreatedAtMs(System.currentTimeMillis());
					}
					AppErrorLog[] logArr3 = logEntity.getAppErrorLogs();
					for (AppErrorLog log : logArr3) {
						PropertiesUtil.copyProperties(logEntity, log);
						log.setCreatedAtMs(System.currentTimeMillis());
					}
					AppPageLog[] logArr4 = logEntity.getAppPageLogs();
					for (AppPageLog log : logArr4) {
						PropertiesUtil.copyProperties(logEntity, log);
						log.setCreatedAtMs(System.currentTimeMillis());
					}
					//将对象转换成json string
